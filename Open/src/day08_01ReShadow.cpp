...
void DrawShadow(void) {
	...
	glStencilFunc(GL_ALWAYS, 1, ~0);  // 基準値を1、すべて合格
	//これから描画するもの(s,dpをpassしたもの)のステンシル値をすべて1にする
	...
	glColorMask(0, 0, 0, 0);  // カラーバッファを保護
	glDepthMask(0);           // デプスバッファを保護
	{
...
//		glEnable(GL_STENCIL_TEST);
		...
		glPushMatrix();
		glMultMatrixf(pM);
		DrawStructure(false);  // 物体が床より上なら影ステンシルをdraw
		glPopMatrix();
	}
	glColorMask(1, 1, 1, 1);
	glDepthMask(1);
...
//	void glStencilFunc(
//		GLenum func,  // 比較方法を指定
//		GLint ref,    // 基準値を指定(ステンシルテストを通過させる値)
//		GLuint mask   // ~0(全ビット1)にしておけば無視できる
//	);
//	GL_NEVER：すべて不合格
...
//	void glStencilOp(
//		GLenum sfail,   // ステンシルテストを通過できなかった時の処理
//		GLenum dpfail,  // ステンシルテストは通過できたが、デプステストを通過できなかった時の処理
//		GLenum dppass   // どちらも通過した時の処理
//	);
//	GL_KEEP : 現在の値をキープする。
